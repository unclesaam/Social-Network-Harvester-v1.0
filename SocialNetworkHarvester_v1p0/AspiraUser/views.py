...
def addRemoveItemById(request, addRemove):
    ...
    if not model.objects.filter(pk=id).exists():return jsonBadRequest('item #%s of type "%s" does not exists'%(id, model))
    ...
    if addRemove == 'add':
        ...
        list.add(item)
        return jResponse({'message':{"code":200, "message":"<b>%s</b> as été ajouté de votre liste de collecte."%item}})
    else:
        if not item in list.all(): return jsonBadRequest("%s is not in current list"%item)
        list.remove(item)
        return jResponse({'message':{"code":200, "message":"<b>%s</b> as été retiré de votre liste de collecte."%item}})
